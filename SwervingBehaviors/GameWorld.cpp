            ...
            m_bShowCellSpaceInfo(false)
{
  ...
  for (int a=0; a<Prm.NumAgents; ++a)
  {
	...
	Vector2D SpawnPos2 = Vector2D(cx / 5.0 + RandomClamped()*cx / 5.0,
		cy / 5.0 + RandomClamped()*cy / 5.0);
 ...
 //   m_pCellSpace->AddEntity(pVehicle2);

	//pVehicle2->Steering()->FlockingOn();
...
#define SHOAL
  ...
  m_Vehicles[Prm.NumAgents-1]->SetScale(Vector2D(20,15));
  m_Vehicles[Prm.NumAgents-1]->Steering()->WanderOn();
  m_Vehicles[Prm.NumAgents-1]->SetMaxSpeed(100);
  //2nd vehic acting as body guard. It's supposed to pursue/follow the primary vehic
  m_Vehicles[Prm.NumAgents - 2]->Steering()->FlockingOff();
  m_Vehicles[Prm.NumAgents - 2]->SetScale(Vector2D(10, 15));
  m_Vehicles[Prm.NumAgents - 2]->Steering()->PursuitOn(m_Vehicles[Prm.NumAgents - 1]);
  m_Vehicles[Prm.NumAgents - 2]->SetMaxSpeed(90);


  // for (int i=0; i<Prm.NumAgents-1; ++i)
  //{
  //  m_Vehicles[i]->Steering()->PursuitOn(m_Vehicles[Prm.NumAgents-2]);
